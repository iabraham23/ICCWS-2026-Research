...
class GraphRAGPipeline:
    ...
    def _extract_entities(self, input_dict):
        ...
        question = f"A students bash commands: {bash}\n A students chat messages: {chat} A students recent attempts: {answers}"
    ...
    def _neighborhood_expansion(self, names):
        cypher_exact = """
    MATCH path = (n)-[*1..2]->(m)
    WHERE 
    ...
    def _query_graph(self, input_dict):
        ...
        if len(raw_results)<5: #look at lexical AND cosine similarity
            ...
            exact_results = self._neighborhood_expansion(resolved_names)
            exact_results = [{**res, "source": "neighbordhood"} for res in exact_results]
            ...
            semantic_results = self._neighborhood_expansion(semantic_nodes)
        ...
        if len(graph_results)<2: #incorrect way to return direct, we can fix later
        ...
        if len(input_dict["chat_messages"])==0: #if they haven't asked any questions, don't score
            return {"graph_results": graph_results[:4], 
                "bash_commands": input_dict["bash_commands"], 
                "chat_messages": input_dict["chat_messages"], 
                "chat_answers": input_dict["chat_answers"]
                }
        
        print(f'length of chat messages {len(input_dict["chat_messages"])}')
    ...
    def _build_summarize_chain(self): #for now just copy and past getting started in
        summary_prompt = PromptTemplate.from_template(
            ...
            "The student's recent bash commands: {bash_commands}, The student's recent chat messages: {chat_messages}, The students recent attempts: {chat_answers}"
